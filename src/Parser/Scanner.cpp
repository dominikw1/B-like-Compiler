...
constexpr bool isNextChar(auto curr, auto end, char c) {
    if (curr + 1 == end) {
        return false;
    }
    return *(curr + 1) == c;
...
constexpr std::optional<std::string_view> tryParseExactMatch(auto& curr, auto end, std::string_view match) {
    auto helper = curr;
    for (auto& c : match) {
...
constexpr std::optional<Token> tryParseIdentifier(auto& curr, auto end) {
    auto it = curr;
...
constexpr std::string_view incrementCurr(auto& currIt, std::size_t len) {
    auto ret = std::string_view{currIt, len};
    currIt += len;
    return ret;
}

constexpr std::optional<Token> lexNextToken(auto& curr, auto end) {
    if (curr == end)
        return {};
    if (isNumber(*curr)) {
